new Vue({
  // elプロパティでvueの表示を反映する場所=HTML要素のセレクター(id)を定義する
  el: '#app',
  ...
  computed: {
    // 商品情報の状態一覧を表示する
    labels(){
      return this.option.reduce(function (a,b) {
    ...
    doFetchProduct(product) {
      ...
      .then(response => {
        if (response.status != 200) {
          ...
          var index = this.doFetchAllProducts.indexOf(product)
          // spliceを使うとdataプロパティの配列の要素をリアクティブに変更できる
          this.doFetchAllProducts.splice(index, 1, resultProduct[0])
        }
      })
    },
    // 商品情報を登録する
